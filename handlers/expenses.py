...
async def start_add_expense(callback: CallbackQuery, state: FSMContext, session: AsyncSession, current_user: User, **kwargs):
    ...
    await callback.message.answer(
        "➕ <b>Новая трата</b>\n\n"
        "Напиши одним сообщением <b>описание и сумму</b>:\n\n"
        "Примеры:\n"
        "• <code>Обед 1500</code>\n"
        "• <code>Такси 350</code>\n"
...
async def expense_currency(callback: CallbackQuery, state: FSMContext, session: AsyncSession, current_user: User, **kwargs):
    ...
    if len(members) == 1:
        ...
        await callback.message.answer("🗂️ Выбери категорию:", reply_markup=category_keyboard())
    else:
        await state.set_state(ExpenseStates.payer)
        await callback.message.answer("👤 Кто платил?", reply_markup=payer_keyboard(members, current_user.id))
...
async def expense_category(callback: CallbackQuery, state: FSMContext, session: AsyncSession, current_user: User, **kwargs):
    ...
    if len(members) == 1:
        ...
        await _save_expense(callback.message, state, session, current_user, edit=False)
    else:
...
async def _show_expense_page(callback, session, trip_id, page, current_user):
    ...
    if not expenses:
        await callback.message.answer("📋 Трат пока нет!", reply_markup=trip_menu_keyboard(trip_id))
        return
    ...
    builder = InlineKeyboardBuilder()
    for exp in chunk:
        builder.button(text=f"🗑 #{exp.id}", callback_data=f"delete_exp:{exp.id}:{trip_id}:{page}")
        builder.button(text=f"✅ погашено #{exp.id}", callback_data=f"paid_exp:{exp.id}:{current_user.id}:{trip_id}:{page}")
    builder.adjust(2)
    if page > 0:
